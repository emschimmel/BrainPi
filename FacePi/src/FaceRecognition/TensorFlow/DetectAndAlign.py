...
class DetectAndAlign():
    @classmethod
    def align_image(self, img, pnet, rnet, onet):
        margin = 44
        image_size = 160

        img_size = np.asarray(img.shape)[0:2]
        bounding_boxes, landmarks = self.detect_face(img, pnet, rnet, onet)
    ...
    def create_mtcnn(self, sess, model_path):
        ...
        with tf.variable_scope('pnet'):
            ...
            pnet = self.PNet({'data': data})
            pnet.load(os.path.join(model_path, 'det1.npy'), sess)
        with tf.variable_scope('rnet'):
            data = tf.placeholder(tf.float32, (None, 24, 24, 3), 'input')
            rnet = self.RNet({'data': data})
            rnet.load(os.path.join(model_path, 'det2.npy'), sess)
        with tf.variable_scope('onet'):
            data = tf.placeholder(tf.float32, (None, 48, 48, 3), 'input')
            onet = self.ONet({'data': data})
    ...
    @classmethod
    def detect_face(self, img, pnet, rnet, onet):
